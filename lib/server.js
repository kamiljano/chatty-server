...
module.exports.Server = class {
  ...
  constructor({port = 5555, corsOrign = '*'}) {
    this.port = port;
    this.corsOrign = corsOrign;
  }
  ...
  start() {
    return new Promise(resolve => {
      ...
      router.use((req, res, next) => {
        res.header("Access-Control-Allow-Origin", this.corsOrign);
      ...
      this._server = server.listen(this.port, () => {
        console.info(`Listening on localhost:${this.port}. CORS set up for UI hosted at ${this.corsOrign}`);
        resolve();
      });
    });
  }
